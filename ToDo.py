...
def index():
    db, c = get_db()
    c.execute(
        'SELECT t.id, t.description, u.username, t.completed, t.created_at FROM todo t JOIN user u on t.created_by = u.id order '
        'where t.created_by = %s by created_at desc'
    , (g.user['id'],)).fetchall()
    todos = c.fetchall()
